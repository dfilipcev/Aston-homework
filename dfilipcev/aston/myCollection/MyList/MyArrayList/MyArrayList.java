...
public class MyArrayList<E> implements MyList<E> {
    ...
    public void sort(Comparator<? super E> comparator) {
        ...
        qSort(from, to, comparator);
    }

    private void qSort(int from, int to, Comparator<? super E> comparator) {
        if (from < to) {
            int divide = partition(from, to, comparator);
            qSort(from, divide - 1, comparator);
            qSort(divide, to, comparator);
        }
    }

    @SuppressWarnings("unchecked")
    private int partition(int from, int to, Comparator<? super E> comparator) {
        int leftIndex = from;
        int rightIndex = to;

        E pivot = (E) elementsData[from];

        while (leftIndex <= rightIndex) {
            while(comparator.compare((E) elementsData[leftIndex], pivot) < 0) {
                leftIndex++;
            }
            while(comparator.compare(pivot, (E) elementsData[rightIndex]) < 0) {
                rightIndex--;
            }
    ...
    private void swap(int leftIndex, int rightIndex) {
        ...
        Object temp = elementsData[leftIndex];
        elementsData[leftIndex] = elementsData[rightIndex];
        elementsData[rightIndex] = temp;
    }
